...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("LiviaCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #LiviaCoinTEST2\r");
            Send(hSocket, "WHO #LiviaCoinTEST2\r");
        } else {
            // randomly join #LiviaCoin00-#LiviaCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #LiviaCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #LiviaCoin%02d\r", channel_number).c_str());
        }
