LiviaCoin-qt: Qt4 GUI for LiviaCoin
===============================
...
Alternatively, install Qt Creator and open the `LiviaCoin-qt.pro` file.

An executable named `LiviaCoin-qt` will be built.
...
A warning for people using the *static binary* version of LiviaCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of LiviaCoin is linked against libdb5.3.
...
installed causes LiviaCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
